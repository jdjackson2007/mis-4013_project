    ...
    <div class="row mt-5">
        <div class="col">
            ...
            <div class="oath-box bg-secondary p-4 rounded">
                <?php 
                // Fetch the data again to avoid reusing the exhausted result set
                $corpsList->data_seek(0); // Reset result pointer
                while ($corps = $corpsList->fetch_assoc()): ?>
                    <p>
                        <strong class="text-warning"><?php echo htmlspecialchars($corps['Corps_Name']); ?> Oath:</strong>
                        <?php echo htmlspecialchars($corps['Corps_Oath']); ?>
                    </p>
                <?php endwhile; ?>
            </div>
        </div>
